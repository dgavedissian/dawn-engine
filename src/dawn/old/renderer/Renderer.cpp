...
Renderer::Renderer(const String& basePath, const String& prefPath, Input* inputMgr,
    ...
    mRenderWindow =
        mRoot->createRenderWindow(windowTitle, displayMode.x, displayMode.y, fullscreen, &options);
...
void Renderer::createSDLWindow(const String& windowTitle, const Vec2i& displayMode, bool fullscreen,
                               Ogre::NameValuePairList& options) {
    // Create the window
    int windowFlags = 0;
    if (fullscreen)
    ...
    for (uint i = 0; i < mesh->getNumSubMeshes(); ++i) {
        ...
        for (uint k = 0; k < subMesh->indexData->indexCount; k++) {
            ...
            switch (k % 3) {
                ...
                case 2:
                    rayToTriangleCheck(vertex1, vertex2, vertexPos, closestDistance, position,
                                       normal);
                    break;
