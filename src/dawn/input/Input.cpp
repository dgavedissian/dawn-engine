...
void Input::handleSDLEvent(SDL_Event& e) {
    switch (e.type) {
        ...
        case SDL_KEYUP:
            EventSystem::inst().queueEvent(makeShared<EvtData_KeyUp>(
                e.key.keysym.sym, e.key.keysym.scancode, e.key.keysym.mod));
            break;
        ...
        case SDL_MOUSEMOTION: {
            ...
            EventSystem::inst().queueEvent(
                makeShared<EvtData_MouseMove>(pos, posRel, Vec2i(e.motion.xrel, e.motion.yrel)));
        } break;
